...
import com.ordis.game.Game.TikTakToe;
...
public class Bot {
    private final Map<CommandData, Consumer<SlashCommandInteractionEvent>> data = Map.of(
            ...
            // tik tak toe
            new CommandDataImpl("tik-tak-toe", "starts a tik-tak-toe game"),
            (SlashCommandInteractionEvent event) -> {
                event.reply("I am starting a new game for you ...").queue();
                new TikTakToe(event.getTextChannel(), event.getUser()).start();
            },
            // privacy
